          ...
          {filteredPersons.length === 0 ? (
              ...
              <ul className="divide-y divide-gray-100 dark:divide-gray-700">
                {filteredPersons.map((person) => (
                  <li key={person} className="flex items-center justify-between py-2.5 px-4 gap-2">
                    <span className="flex items-center gap-2.5 text-sm font-medium text-gray-900 dark:text-gray-100 min-w-0">
                      <svg xmlns="http://www.w3.org/2000/svg" className="w-4 h-4 text-gray-400 dark:text-gray-500 shrink-0 self-center" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round">
                        <circle cx="12" cy="8" r="4" />
                        <path d="M4 20c0-4 3.6-7 8-7s8 3 8 7" />
                      </svg>
                      <span className="inline-flex items-center h-4 truncate translate-y-px">{person}</span>
                    </span>
                    <button
